...
class ProgramNode extends ASTnode {
    ...
    public void nameAnalysis() {
        ...
        int offset = 0;
        boolean atGlobal = true;
        myDeclList.nameAnalysis(symTab, offset, atGlobal);

        Sym main = symTab.lookupGlobal("main");
...
class DeclListNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, int offset, boolean atGlobal) {
        nameAnalysis(symTab, symTab, offset, atGlobal);
    }
     ...
     * decls in the list.
     */    
    public void nameAnalysis(SymTable symTab, SymTable globalTab, int offset, boolean atGlobal) {
        for (DeclNode node : myDecls) {
            if (node instanceof VarDeclNode) {
                if (atGlobal) {
                    ((VarDeclNode)node).nameAnalysis(symTab, globalTab, 0);
                } else {
...
class FormalsListNode extends ASTnode {
    ...
    public List<Type> nameAnalysis(SymTable symTab, int offset) {
        ...
        for (FormalDeclNode node : myFormals) {
            Sym sym = node.nameAnalysis(symTab, offset);
            offset += 4;
            if (sym != null) {
...
class FnBodyNode extends ASTnode {
    ...
    public int nameAnalysis(SymTable symTab, int offset) {
        myDeclList.nameAnalysis(symTab, offset, false);
        offset = offset - myDeclList.getSize() * 4;
...
class ExpListNode extends ASTnode {
    ...
    public void codeGen() {
        //generate backwards so that the top of stack would be the first parameter
        for (int i = myExps.size() - 1; i >= 0; i--) {
            myExps.get(i).codeGen();
        }
    }

    public void codeGenClean() {
        //generate backwards so that the top of stack would be the first parameter
        for (int i = myExps.size() - 1; i >= 0; i--) {
...
abstract class DeclNode extends ASTnode {
     ...
     */
    abstract public Sym nameAnalysis(SymTable symTab, int offset);
...
class VarDeclNode extends DeclNode {
    ...
    public void codeGen() {
        ...
        if (myId.sym().offset() == 0) {
            ...
            Codegen.generateLabeled("_" + myId.name(), ".word", "Global variable " + myId.name(), "0");
        } else {
            myId.codeGenAddr();
            Codegen.genPop(Codegen.T0);
...
class FnDeclNode extends DeclNode {
    ...
    public Sym nameAnalysis(SymTable symTab, int offset) {
        ...
        // process the formals
        List<Type> typeList = myFormalsList.nameAnalysis(symTab, offset + 4);
        if (sym != null) {
            sym.addFormals(typeList);
        }
        
        offset = myBody.nameAnalysis(symTab, offset - 8); // process the function body
        
        if (sym != null) {
            sym.setLocalVarSize(-(offset + 8));
        }
    ...
    public void unparse(PrintWriter p, int indent) {
        ...
        //test offset
        p.print(  ((FnSym)(myId.sym())).localVarSize() );
    ...
    public void codeGen() {
        ...
        Codegen.generate("subu", Codegen.SP, Codegen.SP, ((FnSym)(myId.sym())).localVarSize());
...
class StructDeclNode extends DeclNode {
    ...
    public Sym nameAnalysis(SymTable symTab, int offset) {
        ...
        //don't care about offset in struct for this project
        myDeclList.nameAnalysis(structSymTab, symTab, 0, true);
...
class WriteStmtNode extends StmtNode {
    ...
    public void codeGen(String retLabel) {
        Codegen.generate("#WRITE STMT");
        myExp.codeGen();
        Codegen.genPop(Codegen.A0);
        String mode = myExp.typeCheck().isStringType() ? "4" : "1";
        Codegen.generate("li", Codegen.V0, mode);
        Codegen.generate("syscall");
    }
...
class IfStmtNode extends StmtNode {
    ...
    public void codeGen(String retLabel) {
        ...
        Codegen.genPop(Codegen.T0);
        Codegen.generate("li", Codegen.T1, 0);
        Codegen.generate("beq", Codegen.T0, Codegen.T1, end);

        //fall through
        myDeclList.codeGen();
        myStmtList.codeGen(retLabel);

        Codegen.genLabel(end, "IF END");
...
class IfElseStmtNode extends StmtNode {
    ...
    public int nameAnalysis(SymTable symTab, int offset) {
        myExp.nameAnalysis(symTab);
        symTab.addScope();
        myThenDeclList.nameAnalysis(symTab, offset, false);
        offset = offset - myThenDeclList.getSize() * 4;
        ...
        symTab.addScope();
        myElseDeclList.nameAnalysis(symTab, offset, false);
        offset = offset - myElseDeclList.getSize() * 4;
    ...
    public void codeGen(String retLabel) {
        Codegen.generate("#IF_ELSE STMT");
        
        myExp.codeGen();
        String else_label = Codegen.nextLabel();
        String end = Codegen.nextLabel();
        //check condition
        Codegen.genPop(Codegen.T0);
        Codegen.generate("li", Codegen.T1, 0);
        Codegen.generate("beq", Codegen.T0, Codegen.T1, else_label);
        ...
        //else branch
        Codegen.genLabel(else_label, "ELSE BLOCK");
        myElseDeclList.codeGen();
        myElseStmtList.codeGen(retLabel);

        Codegen.genLabel(end, "IF_ELSE END");
...
class WhileStmtNode extends StmtNode {
    ...
    public void codeGen(String retLabel) {
        ...
        String check = Codegen.nextLabel();
        String end = Codegen.nextLabel();
        //check condition
        Codegen.genLabel(check, "WHILE CHECK");
        myExp.codeGen();
        Codegen.genPop(Codegen.T0);
        Codegen.generate("li", Codegen.T1, 0);
        Codegen.generate("beq", Codegen.T0, Codegen.T1, end);
        ...
        myDeclList.codeGen();
        myStmtList.codeGen(retLabel);
        Codegen.generate("j", check);

        Codegen.genLabel(end, "WHILE END");
...
class RepeatStmtNode extends StmtNode {
    ...
    public void codeGen(String retLabel) {
        Codegen.generate("#REPEAT STMT NOT IMPLEMENTED");
    }
...
class StringLitNode extends ExpNode {
    ...
    public void codeGen() {
        if (usedLabel.get(myStrVal) == null) {
            Codegen.generate(".data");
            ...
            Codegen.genPush(Codegen.T0);
            usedLabel.put(myStrVal, label);
        } else {
            Codegen.generate("la", Codegen.T0, usedLabel.get(myStrVal));
            Codegen.genPush(Codegen.T0);  
        }
    ...
    private static Map<String, String> usedLabel = new HashMap<>();
}
    ...
    public void codeGenAddr() {
        if (!mySym.getType().isFnType()) {
            if (mySym.offset() == 0) {
                ...
                Codegen.generate("addu", Codegen.T0, Codegen.FP, mySym.offset());
            }
            Codegen.genPush(Codegen.T0);
        }
    }
...
class AssignNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        //leave value on stack, since assign can be cascade
        Codegen.genPush(Codegen.T0);
    }
...
class CallExpNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        //returned, clean parameters from stack
        if (myExpList != null) {
            myExpList.codeGenClean();
        }
...
class NotNode extends UnaryExpNode {
    ...
    public void codeGen() {
        ...
        String push1 = Codegen.nextLabel();
        String end = Codegen.nextLabel();
        ...
        Codegen.generate("li", Codegen.T1, "0");
        Codegen.generate("beq", Codegen.T0, Codegen.T1, push1);

        //fall through, push0
        Codegen.generate("j", end);

        Codegen.genLabel(push1);
...
class TimesNode extends ArithmeticExpNode {
    ...
    public void codeGen() {
        ...
        //put 32 least significant bits of multiplication to $t0
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        //fall through only if exp1 is true, then ans = exp2
        myExp2.codeGen();
        Codegen.genPop(Codegen.T0);
...
class OrNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        //fall through only if exp1 is false, then ans = exp2
        myExp2.codeGen();
        Codegen.genPop(Codegen.T0);
